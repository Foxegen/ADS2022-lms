...
public class A_LIS {
    ...
    int getSeqSize(InputStream stream) {
        ...
        int[] dp = new int[n]; // массив для хранения длин LIS до каждого элемента

        for (int i = 0; i < n; i++) {
            dp[i] = 1; // начальная длина подпоследовательности для каждого элемента
            for (int j = 0; j < i; j++) {
        ...
        for (int i = 0; i < n; i++) {
            if (max < dp[i]) {
                max = dp[i]; // находим максимальную длину LIS
            }
        }
